...
void Combat::getCombatArea(
	const Position& centerPos,
	const Position& targetPos,
	const CombatArea* area,
	std::list<Tile*>& list
) {
	if (area) {
...
ReturnValue Combat::canDoCombat(
	...
	if (const Player* targetPlayer = target->getPlayer()) {
		...
		if ((attackerPlayer = attacker->getPlayer()) || (attackerPlayer = attacker->getPlayerMaster())) {
			...
			if ((g_game.getWorldType() == WORLDTYPE_OPTIONAL && !Combat::isInPvpZone(attacker, target)
				...
				#endif
			) || isProtected(const_cast<Player*>(attackerPlayer), const_cast<Player*>(targetPlayer)) || (g_config.getBool(ConfigManager::CANNOT_ATTACK_SAME_LOOKFEET) && attackerPlayer->getDefaultOutfit().lookFeet == targetPlayer->getDefaultOutfit().lookFeet) || !attackerPlayer->canSeeCreature(targetPlayer)) {
				return RET_YOUMAYNOTATTACKTHISPLAYER;
			}
		}
...
void Combat::setPlayerCombatValues(
	formulaType_t _type,
	double _mina,
	double _minb,
	double _maxa,
...
bool Combat::CombatConditionFunc(
	Creature* caster,
	Creature* target,
	const CombatParams& params,
	void*
...
void Combat::addDistanceEffect(
	Creature* caster,
	const Position& fromPos,
	const Position& toPos,
	ShootEffect_t effect
) {
...
void ValueCallback::getMinMaxValues(
	Player* player,
	int32_t& min,
	int32_t& max,
	bool useCharges
...
void TileCallback::onTileCombat(
	Creature* creature,
	Tile* tile
) const {
...
void TargetCallback::onTargetCombat(
	Creature* creature,
	Creature* target
) const {
...
void CombatArea::copyArea(
	const MatrixArea* input,
	MatrixArea* output,
	...
	if (op == MATRIXOPERATION_COPY) {
		...
		for (int32_t x = 0; x < (long)input->getCols(); ++x) {
			for (int32_t y = 0; y < (long)input->getRows(); ++y) {
				...
				int32_t newX = x - centerX, newY = y - centerY, rotatedX = round(newX * a + newY * b), rotatedY = round(newX * c + newY * d);
...
MatrixArea* CombatArea::createArea(
	const std::list<uint32_t>& list,
	uint32_t rows
) {
...
void MagicField::onStepInField(
	...
	if (ownerId && !getTile()->hasFlag(TILESTATE_HARDCOREZONE)) {
		if (Creature* owner = g_game.getCreatureByID(ownerId)) {
			bool harmful = true;
			if ((g_game.getWorldType() == WORLDTYPE_OPTIONAL || getTile()->hasFlag(TILESTATE_OPTIONALZONE)) && (owner->getPlayer() || owner->isPlayerSummon())) {
				harmful = false;
