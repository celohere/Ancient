...
void Player::setConditionSuppressions(
	uint32_t conditions,
	bool remove
) {
	if (remove) {
...
void Player::getShieldAndWeapon(
	const Item* &_shield,
	const Item* &_weapon
) const {
	_shield = NULL;
...
void Player::addSkillAdvance(
	...
	uint64_t currReqTries = vocation->getReqSkillTries(skill, skills[skill][SKILL_LEVEL]), nextReqTries = vocation->getReqSkillTries(skill, skills[skill][SKILL_LEVEL] + 1);
...
bool Player::setStorage(
	...
	if (IS_IN_KEYRANGE(numericKey, OUTFITS_RANGE)) {
		...
		if (addons < 4) {
			...
			std::clog << "[Warning - Player::setStorage] Invalid addons value key: " << key << ", value: " << value << " for player: " << getName() << std::endl;
		}
...
bool Player::canWalkthrough(const Creature* creature) const {
	if (creature == this || hasCustomFlag(PlayerCustomFlag_CanWalkthrough) || creature->isWalkable() || (creature->getMaster() && creature->getMaster() != this && canWalkthrough(creature->getMaster()))) {
		return true;
	}
	...
	player->getVocation()->isAttackable()) || (player->getVocation()->isAttackable() && player->getLevel() < (uint32_t)g_config.getNumber(ConfigManager::PROTECTION_LEVEL))) && player->getTile()->ground && Item::items[player->getTile()->ground->getID()].walkStack) && (!player->hasCustomFlag(PlayerCustomFlag_GamemasterPrivileges) || player->getAccess() <= getAccess())) {
		return true;
	}
	return (player->isGhost() && getGhostAccess() < player->getGhostAccess()) || (isGhost() && getGhostAccess() > player->getGhostAccess());
}

Depot* Player::getDepot(
	uint32_t depotId,
...
void Player::sendCreatureChangeVisible(
...
void Player::sendAddContainerItem(
...
void Player::sendRemoveContainerItem(
	const Container* container,
	uint8_t slot,
	const Item*
) {
...
bool Player::canShopItem(
	uint16_t itemId,
	uint8_t subType,
	ShopEvent_t event
) {
	for (ShopInfoList::iterator sit = shopOffer.begin(); sit != shopOffer.end(); ++sit) {
		if (sit->itemId != itemId || ((event != SHOPEVENT_BUY || sit->buyPrice < 0) && (event != SHOPEVENT_SELL || sit->sellPrice < 0))) {
			continue;
		}
...
void Player::onAddContainerItem(
	const Container*,
	const Item* item
) {
	checkTradeState(item);
...
void Player::onUpdateContainerItem(
	const Container* container,
	uint8_t slot,
	const Item* oldItem,
	const ItemType&,
...
void Player::onRemoveContainerItem(
	const Container* container,
	uint8_t,
	const Item* item
) {
...
void Player::onUpdateInventoryItem(
	slots_t slot,
	Item* oldItem,
	const ItemType&,
	Item* newItem,
...
uint16_t Player::getPercentLevel(
	uint64_t count,
...
void Player::onAttackedCreatureBlockHit(
	Creature* target,
...
BlockType_t Player::blockHit(
	...
	if (vocation->getMultiplier(MULTIPLIER_MAGICDEFENSE) != 1.0 && combatType != COMBAT_NONE && combatType != COMBAT_PHYSICALDAMAGE && combatType != COMBAT_UNDEFINEDDAMAGE && combatType != COMBAT_DROWNDAMAGE) {
...
ReturnValue Player::__queryAdd(
		...
		case SLOT_RIGHT: {
			if (item->getSlotPosition() & SLOTP_RIGHT) {
				...
				if (item->getSlotPosition() & SLOTP_TWO_HAND) {
					...
					} else if (!leftItem->isWeapon() || !item->isWeapon() || leftType == WEAPON_AMMO || type == WEAPON_AMMO || leftType == WEAPON_SHIELD || type == WEAPON_SHIELD || (leftItem->isDualWield() && item->isDualWield())) {
						ret = RET_NOERROR;
					} else {
			...
			if (item->getSlotPosition() & SLOTP_LEFT) {
				...
				if (item->getSlotPosition() & SLOTP_TWO_HAND) {
					...
					if (rightItem->getSlotPosition() & SLOTP_TWO_HAND) {
						...
						ret = RET_CANONLYUSEONESHIELD;
					} else if (!rightItem->isWeapon() || !item->isWeapon() || rightType == WEAPON_AMMO || type == WEAPON_AMMO || rightType == WEAPON_SHIELD || type == WEAPON_SHIELD || (rightItem->isDualWield() && item->isDualWield())) {
						ret = RET_NOERROR;
					} else {
						ret = RET_CANONLYUSEONEWEAPON;
					}
	...
	if (ret == RET_NOERROR || ret == RET_NOTENOUGHROOM) {
		...
		if (getInventoryItem((slots_t)index) != NULL && (!getInventoryItem((slots_t)index)->isStackable() || getInventoryItem((slots_t)index)->getID() != item->getID())) {
...
ReturnValue Player::__queryMaxCount(
	int32_t index,
	const Thing* thing,
	uint32_t count,
...
ReturnValue Player::__queryRemove(
	const Thing* thing,
	uint32_t count,
	uint32_t flags
) const {
...
Cylinder* Player::__queryDestination(
	int32_t& index,
	const Thing* thing,
	Item** destItem,
	uint32_t& flags
...
uint32_t Player::__getItemTypeCount(
	uint16_t itemId,
	int32_t subType
) const {
	Item* item = NULL;
...
void Player::postAddNotification(
	Creature*,
	Thing* thing,
...
void Player::postRemoveNotification(
	Creature*,
	Thing* thing,
...
void Player::__internalAddThing(
	uint32_t index,
	Thing* thing
) {
	#ifdef __DEBUG_MOVESYS__
...
void Player::getPathSearchParams(
	const Creature* creature,
	FindPathParams& fpp
) const {
...
double Player::getGainedExperience(Creature* attacker) const {
	...
	if ((min > 0 && level < (uint32_t)std::floor(attackerLevel * min)) || (max > 0 && level > (uint32_t)std::floor(attackerLevel * max))) {
		return 0;
	}
...
void Player::onCombatRemoveCondition(
	const Creature*,
	Condition* condition
) {
...
void Player::onSummonAttackedCreature(
	Creature* summon,
	Creature* target
) {
...
bool Player::checkLoginDelay(uint32_t playerId) const {
	return (!hasCustomFlag(PlayerCustomFlag_IgnoreLoginDelay) && OTSYS_TIME() <= (lastLoad + g_config.getNumber(ConfigManager::LOGIN_PROTECTION)) && !hasBeenAttacked(playerId));
}
...
void Player::onAttackedCreatureDrain(
	Creature* target,
	int32_t points
) {
...
void Player::onSummonAttackedCreatureDrain(
	Creature* summon,
	Creature* target,
	int32_t points
) {
...
void Player::onTargetCreatureGainHealth(
	Creature* target,
	int32_t points
) {
...
bool Player::onKilledCreature(
	...
	if (target->getMonster() && !target->isPlayerSummon() && !hasFlag(PlayerFlag_HasInfiniteStamina) && (condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_HUNTING, g_config.getNumber(ConfigManager::HUNTING_DURATION)))) {
		addCondition(condition);
	}
	...
	if (!targetPlayer->hasAttacked(this) && target->getSkull() == SKULL_NONE && targetPlayer != this && (addUnjustifiedKill(targetPlayer,
...
bool Player::canWearOutfit(
	uint32_t outfitId,
	uint32_t addons
) {
	OutfitMap::iterator it = outfits.find(outfitId);
	if (it == outfits.end() || (it->second.isPremium && !isPremium()) || getAccess() < it->second.accessLevel || ((it->second.addons & addons) != addons && !hasCustomFlag(PlayerCustomFlag_CanWearAllAddons))) {
		return false;
	}
...
bool Player::addUnjustifiedKill(
	const Player* attacked,
	bool countNow
) {
	if (!g_config.getBool(ConfigManager::USE_FRAG_HANDLER) || hasFlag(PlayerFlag_NotGainInFight) || g_game.getWorldType() != WORLDTYPE_OPEN || hasCustomFlag(PlayerCustomFlag_NotGainUnjustified) || hasCustomFlag(PlayerCustomFlag_NotGainSkull) || attacked == this) {
		return false;
	}
	...
	uint32_t d = g_config.getNumber(ConfigManager::RED_DAILY_LIMIT), w = g_config.getNumber(ConfigManager::RED_WEEKLY_LIMIT), m = g_config.getNumber(ConfigManager::RED_MONTHLY_LIMIT);
	...
	if (!g_config.getBool(ConfigManager::USE_BLACK_SKULL)) {
		...
		if (!IOBan::getInstance()->addAccountBanishment(accountId, (now + g_config.getNumber(ConfigManager::KILLS_BAN_LENGTH)), 20, ACTION_BANISHMENT, "Unjustified player killing.", 0, guid)) {
			return true;
		}
		...
		case MANAGER_NAMELOCK: {
			if (!talkState[1]) {
				...
				if (!IOLoginData::getInstance()->playerExists(managerString, true)) {
					uint32_t tmp;
					if (IOLoginData::getInstance()->getGuidByName(tmp, managerString2) && IOLoginData::getInstance()->changeName(tmp, managerString, managerString2) && IOBan::getInstance()->removePlayerBanishment(tmp, PLAYERBAN_LOCK)) {
			...
			if (checkText(text, "account") && !talkState[1]) {
				...
				if (g_config.getBool(ConfigManager::GENERATE_ACCOUNT_NUMBER)) {
					...
					if (id) {
						...
						msg << "Your account has been created, you may manage it now, but remember your account name: '" << managerChar << "' and password: '" << managerString << "'! If the account name is too hard to remember, please note it somewhere.";
					} else
				...
				if (!IOLoginData::getInstance()->accountNameExists(managerChar)) {
					...
					if (id) {
						...
						noSwap = talkState[1] = false;
						msg << "Your account has been created, you may manage it now, but remember your account name: '" << managerChar << "' and password: '" << managerString << "'!";
					} else {
						msg << "Your account could not be created, please try again.";
					}
