...
*/
	...
	class Service : public ServiceBase {
		public:
			Protocol* makeProtocol(Connection_ptr connection) const {
				return new ProtocolType(connection);
			}
			...
			bool isSingleSocket() const {
				return ProtocolType::isSingleSocket;
			}
			bool hasChecksum() const {
				return ProtocolType::hasChecksum;
			}
	...
	class ServiceManager : boost::noncopyable {
		...
		public:
			ServiceManager():
				m_io_service(), deathTimer(m_io_service), running(false) {}
	...
	bool ServiceManager::add(
		...
		if (it == m_acceptors.end()) {
			...
			if (servicePort->isSingleSocket() || ProtocolType::isSingleSocket) {
				std::clog << "> ERROR: " << ProtocolType::protocolName() << " and " << servicePort->getProtocolNames() << " cannot use the same port (" << port << ")." << std::endl;
				return false;
			}
		}
