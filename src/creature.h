...
*/
	...
	struct DeathEntry {
		DeathEntry(std::string name, int32_t dmg):data(name), damage(dmg), last(false), justify(false), unjustified(false) {}

		DeathEntry(Creature* killer, int32_t dmg):data(killer), damage(dmg), last(false), justify(false), unjustified(false) {}
		...
		bool isUnjustified() const {
			return unjustified;
		}

		const std::type_info& getKillerType() const {
			return data.type();
		}
	...
	class FrozenPathingConditionCall {
		public:
			...
			virtual bool operator()(const Position& startPos, const Position& testPos, const FindPathParams& fpp, int32_t& bestMatchDist) const;
	...
	class Creature : public AutoId, virtual public Thing {
		...
		public:
			...
			bool getHideName() const {
				return hideName;
			}
			void setHideName(bool v) {
				hideName = v;
			}
			...
			void setHideHealth(bool v) {
				hideHealth = v;
			}

			SpeakClasses getSpeakType() const {
				return speakType;
			}
			void setSpeakType(SpeakClasses type) {
				speakType = type;
			}
			...
			void setMasterPosition(const Position& pos, uint32_t radius = 1) {
				masterPosition = pos; masterRadius = radius;
			}
			...
			bool isInvisible() const {
				return hasCondition(CONDITION_INVISIBLE, -1, false);
			}
			...
			inline StorageMap::const_iterator getStorageBegin() const {
				return storageMap.begin();
			}
			inline StorageMap::const_iterator getStorageEnd() const {
			...
			virtual void onTickCondition(ConditionType_t type, int32_t interval, bool& _remov);
			...
			virtual void onCreatureDisappear(const Creature* creature, bool) {
			...
			virtual void onCreatureSay(const Creature*, SpeakClasses, const std::string&, Position* = NULL) {}
			...
			virtual Skulls_t getSkullType(const Creature* creature) const {
				return creature->getSkull();
			}

			virtual void setShield(PartyShields_t newPartyShield) {
				partyShield = newPartyShield;
			}
			virtual PartyShields_t getShield() const {
				return partyShield;
			}
			...
			virtual void setEmblem(GuildEmblems_t newGuildEmblem) {
				guildEmblem = newGuildEmblem;
			}
			virtual GuildEmblems_t getEmblem() const {
				return guildEmblem;
			}
			virtual GuildEmblems_t getGuildEmblem(const Creature* creature) const {
				return creature->getEmblem();
			}

			void setDropLoot(lootDrop_t _lootDrop) {
				lootDrop = _lootDrop;
			}
