...
*/
	...
	class ProtocolManager : public Protocol {
		public:
			...
			ProtocolManager(Connection_ptr connection):
				Protocol(connection) {
			...
			bool checkChannel(uint16_t channelId) const {
				return ((m_channels & (uint32_t)channelId) == (uint32_t)channelId);
			}
