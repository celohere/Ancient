...
*/
	...
	class Player : public Creature, public Cylinder {
		public:
			...
			uint32_t getPromotionLevel() const {
			...
			bool hasFlag(PlayerFlags value) const {
				return group != NULL && group->hasFlag(value);
			}
			...
			bool hasCustomFlag(PlayerCustomFlags value) const {
				return group != NULL && group->hasCustomFlag(value);
			}
			...
			bool hasBlessing(int16_t value) const {
				return (blessings & ((int16_t)1 << value));
			}
			...
			virtual bool isGhost() const {
				return hasCondition(CONDITION_GAMEMASTER, GAMEMASTER_INVISIBLE) || hasFlag(PlayerFlag_CannotBeSeen);
			}
			virtual bool isWalkable() const {
				return hasCustomFlag(PlayerCustomFlag_IsWalkable);
			}
			...
			void setIdleTime(uint32_t amount) {
				idleTime = amount;
			}
			...
			uint16_t getAccess() const {
				return group ? group->getAccess() : 0;
			}
			uint16_t getGhostAccess() const {
				return group ? group->getGhostAccess() : 0;
			}
			...
			uint16_t getSex(bool full) const {
				return full ? sex : sex % 2;
			}
			void setSex(uint16_t);

			uint64_t getStamina() const {
				return hasFlag(PlayerFlag_HasInfiniteStamina) ? STAMINA_MAX : stamina;
			}
			void setStamina(uint64_t value) {
				stamina = std::min((uint64_t)STAMINA_MAX, (uint64_t)std::max((uint64_t)0, value));
			}
			uint32_t getStaminaMinutes() const {
				return (uint32_t)(getStamina() / (uint64_t)STAMINA_MULTIPLIER);
			}
			void setStaminaMinutes(uint32_t value) {
				setStamina((uint64_t)(value * STAMINA_MULTIPLIER));
			}
			void useStamina(int64_t value) {
				stamina = std::min((int64_t)STAMINA_MAX, (int64_t)std::max((int64_t)0, ((int64_t)stamina + value)));
			}
			uint64_t getSpentStamina() {
				return (uint64_t)STAMINA_MAX - stamina;
			}
			...
			Position getLoginPosition() const {
			...
			void sendCreatureImpassable(const Creature* creature) {
				if (client) {
					client->sendCreatureImpassable(creature);
				}
			}
			...
			void sendShop() const {
				if (client) {
					client->sendShop(shopOffer);
				}
			}

			void sendGoods() const {
				if (client) {
					client->sendGoods(shopOffer);
				}
			}
