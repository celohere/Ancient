...
House::House(uint32_t houseId) {
	...
	name = "Ancient headquarter (Flat 1, Area 42)";
	entry = Position();
	id = houseId;
...
void House::removePlayers(bool ignoreInvites) {
	...
	for (HouseTileList::iterator it = houseTiles.begin(); it != houseTiles.end(); ++it) {
		...
		for (CreatureVector::iterator cit = creatures->begin(); cit != creatures->end(); ++cit) {
			if ((player = (*cit)->getPlayer()) && !player->isRemoved() && (ignoreInvites || !isInvited(player))) {
	...
	if (!paid && _time >= (house->getLastWarning() + 86400)) {
		...
		if (warnings < warningsLimit) {
			...
			if (depot && letter) {
				...
				std::stringstream s;
				s << "Warning!\nThe " << period << " rent of " << house->getRent() << " gold for your " << (house->isGuild() ? "guild hall" : "house") << " \"" << house->getName() << "\" has to be paid. Have it within " << (warningsLimit - warnings) << " days or you will lose your " << (house->isGuild() ? "guild hall" : "house") << ".";
