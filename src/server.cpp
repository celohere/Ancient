...
void ServicePort::open(IPAddressList ips, uint16_t port) {
	...
	for (IPAddressList::iterator it = ips.begin(); it != ips.end(); ++it) {
		try {
			Acceptor_ptr tmp(new boost::asio::ip::tcp::acceptor(m_io_service, boost::asio::ip::tcp::endpoint(*it, m_serverPort)));
	...
	if (error) {
		m_logError = false;
		m_pendingStart = true;
		Scheduler::getInstance().addEvent(createSchedulerTask(5000, boost::bind(&ServicePort::services, boost::weak_ptr<ServicePort>(shared_from_this()), pendingIps, m_serverPort)));
	}
}
...
void ServicePort::accept(Acceptor_ptr acceptor) {
	try {
		...
		acceptor->async_accept(*socket, boost::bind(&ServicePort::handle, this, acceptor, socket, boost::asio::placeholders::error));
	} catch(std::exception& e) {
		if (m_logError) {
			LOG_MESSAGE(LOGTYPE_ERROR, e.what(), "NETWORK")
			m_logError = false;
...
void ServicePort::handle(Acceptor_ptr acceptor, boost::asio::ip::tcp::socket* socket, const boost::system::error_code& error) {
	if (!error) {
		...
		if (remoteIp && ConnectionManager::getInstance()->acceptConnection(remoteIp) && (connection = ConnectionManager::getInstance()->createConnection(socket, m_io_service, shared_from_this()))) {
		...
		if (!m_pendingStart) {
			m_pendingStart = true;
			Scheduler::getInstance().addEvent(createSchedulerTask(5000, boost::bind(&ServicePort::service, boost::weak_ptr<ServicePort>(shared_from_this()), acceptor->local_endpoint().address().to_v4(), m_serverPort)));
		}
	}
