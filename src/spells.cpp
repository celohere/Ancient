...
ReturnValue Spells::onPlayerSay(
	Player* player,
	...
	if (!g_config.getBool(ConfigManager::SPELL_NAME_INSTEAD_WORDS)) {
		...
		return g_game.internalCreatureSay(player, type, reWords, player->isGhost()) ? RET_NOERROR : RET_NOTPOSSIBLE;
	}
...
InstantSpell* Spells::getInstantSpellByIndex(
	const Player* player,
	uint32_t index
) {
	uint32_t count = 0;
...
Position Spells::getCasterPosition(
	Creature* creature,
	Direction dir
) {
...
bool CombatSpell::executeCastSpell(
	Creature* creature,
...
ReturnValue Spell::CreateIllusion(
	Creature* creature,
...
bool InstantSpell::canThrowSpell(
...
bool InstantSpell::internalCastSpell(
	Creature* creature,
	const LuaVariant& var
) {
	if (isScripted()) {
...
bool InstantSpell::executeCastSpell(
	Creature* creature,
	const LuaVariant& var
) {
	// onCastSpell(cid, var)
...
bool InstantSpell::SearchPlayer(
	const InstantSpell*,
	Creature* creature,
	const std::string& param
) {
	...
	if (ret == RET_NOERROR) {
		...
		if (position.z != floor) {
			...
			if (!tmpTile || (!tmpTile->ground && !tmpTile->hasProperty(IMMOVABLEBLOCKSOLID))) {
				...
				tmpTile = g_game.getTile(destination);
				if (tile && tmpTile && tmpTile->ground && !tmpTile->hasProperty(IMMOVABLEBLOCKSOLID) && !tmpTile->floorChange() && tile->hasFlag(TILESTATE_HOUSE) == tmpTile->hasFlag(TILESTATE_HOUSE) && tile->hasFlag(TILESTATE_PROTECTIONZONE) == tmpTile->hasFlag(TILESTATE_PROTECTIONZONE)) {
...
bool ConjureSpell::ConjureItem(
	const ConjureSpell* spell,
	Creature* creature,
	const std::string&
) {
	Player* player = creature->getPlayer();
...
ReturnValue RuneSpell::canExecuteAction(
	const Player* player,
	const Position& toPos
) {
	if (player->hasFlag(PlayerFlag_CannotUseSpells)) {
...
bool RuneSpell::internalCastSpell(
	Creature* creature,
	const LuaVariant& var
) {
