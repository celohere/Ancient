...
void Game::start(ServiceManager* servicer) {
	...
	services = servicer;
	if (!g_config.getBool(ConfigManager::GLOBALSAVE_ENABLED) || g_config.getNumber(ConfigManager::GLOBALSAVE_H) < 1 || g_config.getNumber(ConfigManager::GLOBALSAVE_H) > 24 || g_config.getNumber(ConfigManager::GLOBALSAVE_M) < 0 || g_config.getNumber(ConfigManager::GLOBALSAVE_M) > 59) {
		return;
	}
	...
	saveEvent = Scheduler::getInstance().addEvent(createSchedulerTask(timeLeft, boost::bind(&Game::prepareGlobalSave, this)));
}
...
void Game::cleanMapEx(uint32_t& count) {
		...
		if (g_config.getBool(ConfigManager::CLEAN_PROTECTED_ZONES)) {
			for (; it != trash.end(); ++it) {
				...
				while (tile->getItemList() && tit != tile->getItemList()->end()) {
					if ((*tit)->isMovable() && !(*tit)->isLoadedFromMap() && !(*tit)->isScriptProtected()) {
	...
	std::clog << "> CLEAN: Removed " << count << " item" << (count != 1 ? "s" : "") << " from " << tiles << " tile" << (tiles != 1 ? "s" : "");
	if (marked >= 0) {
...
void Game::proceduralRefresh(RefreshTiles::iterator* it) {
	if (!it) {
...
void Game::refreshMap(RefreshTiles::iterator* it, uint32_t limit) {
	...
	Item* item = NULL;
	for (uint32_t cleaned = 0, downItemsSize = 0; (*it) != end && (limit ? (cleaned < limit) : true); ++(*it), ++cleaned) {
		if (!(tile = (*it)->first)) {
			continue;
		}
...
bool Game::playerMoveCreature(uint32_t playerId, uint32_t movingCreatureId, const Position& movingCreaturePos, const Position& toPos, bool delay) {
	...
	if (!player->canDoAction()) {
		...
		SchedulerTask* task = createSchedulerTask(delay, boost::bind(&Game::playerMoveCreature, this, playerId, movingCreatureId, movingCreaturePos, toPos, true));
	...
	if (player != movingCreature) {
		...
		if (!player->hasFlag(PlayerFlag_CanPushAllCreatures)) {
			...
			if (player->getLevel() < protectionLevel && player->getVocation()->isAttackable()) {
				...
				if (movingPlayer && movingPlayer->getLevel() >= protectionLevel && movingPlayer->getVocation()->isAttackable()) {
					player->sendCancelMessage(RET_NOTMOVABLE);
					return false;
				}
			}
...
ReturnValue Game::internalMoveCreature(Creature* creature, Direction direction, uint32_t flags) {
	...
	if (direction < SOUTHWEST && creature->getPlayer()) {
		...
		if (currentPos.z != 8 && creature->getTile()->hasHeight(3)) { // try go up
			if ((!(tmpTile = map->getTile(Position(currentPos.x, currentPos.y, currentPos.z - 1))) || (!tmpTile->ground && !tmpTile->hasProperty(BLOCKSOLID))) && (tmpTile = map->getTile(Position(destPos.x, destPos.y, destPos.z - 1))) && tmpTile->ground && !tmpTile->hasProperty(BLOCKSOLID)) {
...
bool Game::playerMoveItem(uint32_t playerId, const Position& fromPos, uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint8_t count) {
	...
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
		SchedulerTask* task = createSchedulerTask(delay, boost::bind(&Game::playerMoveItem, this, playerId, fromPos, spriteId, fromStackpos, toPos, count));
	...
	if (!Position::areInRange<1, 1, 0>(playerPos, mapFromPos) && !player->hasCustomFlag(PlayerCustomFlag_CanMoveFromFar)) {
		...
		if (getPathToEx(player, item->getPosition(), listDir, 0, 1, true, true)) {
			Dispatcher::getInstance().addTask(createTask(boost::bind(&Game::playerAutoWalk, this, player->getID(), listDir)));
			SchedulerTask* task = createSchedulerTask(std::max((int32_t)SCHEDULER_MINTICKS, player->getStepDuration()), boost::bind(&Game::playerMoveItem, this, playerId, fromPos, spriteId, fromStackpos, toPos, count));
...
bool Game::playerUseItemEx(uint32_t playerId, const Position& fromPos, int16_t fromStackpos, uint16_t fromSpriteId, const Position& toPos, int16_t toStackpos, uint16_t toSpriteId, bool isHotkey) {
	...
	if (ret != RET_NOERROR) {
		if (ret == RET_TOOFARAWAY) {
			...
			if (getPathToEx(player, walkToPos, listDir, 0, 1, true, true, 10)) {
				...
				SchedulerTask* task = createSchedulerTask(400, boost::bind(&Game::playerUseItemEx, this, playerId, itemPos, itemStackpos, fromSpriteId, toPos, toStackpos, toSpriteId, isHotkey));
				player->setNextWalkActionTask(task);
				return true;
			}
	...
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
		SchedulerTask* task = createSchedulerTask(delay, boost::bind(&Game::playerUseItemEx, this, playerId, fromPos, fromStackpos, fromSpriteId, toPos, toStackpos, toSpriteId, isHotkey));
		player->setNextActionTask(task);
		return false;
	}
...
bool Game::playerUseItem(uint32_t playerId, const Position& pos, int16_t stackpos, uint8_t index, uint16_t spriteId, bool isHotkey) {
	...
	if (ret != RET_NOERROR) {
		if (ret == RET_TOOFARAWAY) {
			...
			if (getPathToEx(player, pos, listDir, 0, 1, true, true)) {
				...
				SchedulerTask* task = createSchedulerTask(400, boost::bind(&Game::playerUseItem, this, playerId, pos, stackpos, index, spriteId, isHotkey));
				player->setNextWalkActionTask(task);
				return true;
			}
			ret = RET_THEREISNOWAY;
	...
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
		SchedulerTask* task = createSchedulerTask(delay, boost::bind(&Game::playerUseItem, this, playerId, pos, stackpos, index, spriteId, isHotkey));
		player->setNextActionTask(task);
		return false;
	}
...
bool Game::playerUseBattleWindow(uint32_t playerId, const Position& fromPos, int16_t fromStackpos, uint32_t creatureId, uint16_t spriteId, bool isHotkey) {
	...
	if (ret != RET_NOERROR) {
		if (ret == RET_TOOFARAWAY) {
			...
			if (getPathToEx(player, item->getPosition(), listDir, 0, 1, true, true)) {
				...
				SchedulerTask* task = createSchedulerTask(400, boost::bind(&Game::playerUseBattleWindow, this, playerId, fromPos, fromStackpos, creatureId, spriteId, isHotkey));
				player->setNextWalkActionTask(task);
				return true;
			}
			ret = RET_THEREISNOWAY;
	...
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
		SchedulerTask* task = createSchedulerTask(delay, boost::bind(&Game::playerUseBattleWindow, this, playerId, fromPos, fromStackpos, creatureId, spriteId, isHotkey));
		player->setNextActionTask(task);
		return false;
	}
...
bool Game::playerRotateItem(uint32_t playerId, const Position& pos, int16_t stackpos, const uint16_t spriteId) {
	...
	Item* item = thing->getItem();
	if (!item || item->getClientID() != spriteId || !item->isRoteable() || (item->isLoadedFromMap() && (item->getUniqueId() != 0 || (item->getActionId() != 0 && item->getContainer())))) {
		player->sendCancelMessage(RET_NOTPOSSIBLE);
		return false;
	}
	...
	if (pos.x != 0xFFFF && !Position::areInRange<1, 1, 0>(pos, player->getPosition())) {
		...
		if (getPathToEx(player, pos, listDir, 0, 1, true, true)) {
			...
			SchedulerTask* task = createSchedulerTask(400, boost::bind(&Game::playerRotateItem, this, playerId, pos, stackpos, spriteId));
			player->setNextWalkActionTask(task);
			return true;
		}
...
bool Game::playerRequestTrade(uint32_t playerId, const Position& pos, int16_t stackpos, uint32_t tradePlayerId, uint16_t spriteId) {
	...
	if (!canThrowObjectTo(tradePartner->getPosition(), player->getPosition()) && !player->hasCustomFlag(PlayerCustomFlag_CanThrowAnywhere)) {
	...
	if (!tradeItem || tradeItem->getClientID() != spriteId || !tradeItem->isPickupable() || (tradeItem->isLoadedFromMap() && (tradeItem->getUniqueId() != 0 || (tradeItem->getActionId() != 0 && tradeItem->getContainer())))) {
		player->sendCancelMessage(RET_NOTPOSSIBLE);
		return false;
	}
	...
	if (!Position::areInRange<1, 1, 0>(tradeItem->getPosition(), player->getPosition())) {
		...
		if (getPathToEx(player, pos, listDir, 0, 1, true, true)) {
			...
			SchedulerTask* task = createSchedulerTask(400, boost::bind(&Game::playerRequestTrade, this, playerId, pos, stackpos, tradePlayerId, spriteId));
			player->setNextWalkActionTask(task);
			return true;
		}
	...
	for (std::map<Item*, uint32_t>::const_iterator it = tradeItems.begin(); it != tradeItems.end(); it++) {
		if (tradeItem == it->first || ((container = dynamic_cast<const Container*>(tradeItem)) && container->isHoldingItem(it->first)) || ((container = dynamic_cast<const Container*>(it->first)) && container->isHoldingItem(tradeItem))) {
...
bool Game::playerAcceptTrade(uint32_t playerId) {
	Player* player = getPlayerByID(playerId);
	if (!player || player->isRemoved() || (player->getTradeState() != TRADE_ACKNOWLEDGE && player->getTradeState() != TRADE_INITIATED)) {
		return false;
	}
...
bool Game::playerLookInTrade(uint32_t playerId, bool lookAtCounterOffer, int32_t index) {
	...
	int32_t lookDistance = std::max(std::abs(player->getPosition().x - tradeItem->getPosition().x), std::abs(player->getPosition().y - tradeItem->getPosition().y));
	if (!index) {
...
bool Game::playerSetAttackedCreature(uint32_t playerId, uint32_t creatureId) {
	...
	Dispatcher::getInstance().addTask(createTask(boost::bind(&Game::updateCreatureWalk, this, player->getID())));
	return true;
}
...
bool Game::playerTurn(uint32_t playerId, Direction dir) {
	...
	if (ret != RET_NOTENOUGHROOM && (ret != RET_NOTPOSSIBLE || player->hasCustomFlag(PlayerCustomFlag_CanMoveAnywhere)) && (ret != RET_PLAYERISNOTINVITED || player->hasFlag(PlayerFlag_CanEditHouses))) {
...
bool Game::steerCreature(Creature* creature, const Position& position) {
	...
	if (!Position::areInRange<1, 1, 0>(creature->getPosition(), position)) {
		SchedulerTask* task = createSchedulerTask(std::max((int32_t)SCHEDULER_MINTICKS, creature->getStepDuration()), boost::bind(&Game::steerCreature, this, creature, position));
...
void Game::checkCreatures() {
	checkCreatureEvent = Scheduler::getInstance().addEvent(createSchedulerTask(EVENT_CHECK_CREATURE_INTERVAL, boost::bind(&Game::checkCreatures, this)));
...
bool Game::combatChangeHealth(CombatType_t combatType, Creature* attacker, Creature* target, int32_t healthChange, MagicEffect_t hitEffect, Color_t hitColor, bool force) {
	...
	if (healthChange > 0) {
		...
		if (g_config.getBool(ConfigManager::SHOW_HEALING_DAMAGE) && !target->isGhost() && (g_config.getBool(ConfigManager::SHOW_HEALING_DAMAGE_MONSTER) || !target->getMonster())) {
			char buffer[20];
...
Position Game::getClosestFreeTile(Creature* creature, Position pos, bool extended, bool ignoreHouse) {
	...
	if (Player* player = creature->getPlayer()) {
		for (PairVector::iterator it = relList.begin(); it != relList.end(); ++it) {
			...
			ReturnValue ret = tile->__queryAdd(0, player, 1, FLAG_IGNOREBLOCKITEM);
			if (ret == RET_NOTENOUGHROOM || (ret == RET_NOTPOSSIBLE && !player->hasCustomFlag(PlayerCustomFlag_CanMoveAnywhere)) || (ret == RET_PLAYERISNOTINVITED && !ignoreHouse && !player->hasFlag(PlayerFlag_CanEditHouses))) {
				continue;
			}
		...
		for (PairVector::iterator it = relList.begin(); it != relList.end(); ++it) {
			Tile* tile = NULL;
			if ((tile = map->getTile(Position((pos.x + it->first), (pos.y + it->second), pos.z))) && tile->__queryAdd(0, creature, 1, FLAG_IGNOREBLOCKITEM) == RET_NOERROR) {
				return tile->getPosition();
			}
		}
	}
