...
bool TalkActions::registerEvent(
	Event* event,
	xmlNodePtr p,
	bool override
) {
...
bool TalkAction::houseGuestList(
	Creature* creature,
	const std::string&,
	const std::string&
) {
...
bool TalkAction::houseSubOwnerList(
	Creature* creature,
	const std::string&,
	const std::string&
) {
...
bool TalkAction::thingProporties(
	Creature* creature,
	const std::string&,
	const std::string& param
) {
	...
	for (tokenizer::iterator it = tokens.begin(); it != tokens.end();) {
		...
		if (Item* item = thing->getItem()) {
			...
			} else if (action == "destination" || action == "position" || action == "pos" || action == "dest" || action == "loc" || action == "location") { // TODO: doesn't work
				if (Teleport* teleport = item->getTeleport()) {
					teleport->setDestination(Position(atoi(parseParams(it, tokens.end()).c_str()), atoi(parseParams(it, tokens.end()).c_str()), atoi(parseParams(it, tokens.end()).c_str())));
				}
			} else {
				std::stringstream s;
			...
			if (action == "health") {
				...
				if (action == "fyi") {
					...
					_player->rates[atoi(parseParams(it, tokens.end()).c_str())] = atof(parseParams(it, tokens.end()).c_str());
				} else if (action == "idle") {
					...
					g_talkActions->onPlayerSay(_player, atoi(parseParams(it, tokens.end()).c_str()), parseParams(it, tokens.end()), booleanString(parseParams(it, tokens.end())));
...
bool TalkAction::banishmentInfo(
	Creature* creature,
	const std::string&,
	const std::string& param
) {
	...
	sprintf(buffer, "%s has been %s at:\n%s by: %s,\nfor the following reason:\n%s.\nThe action taken was:\n%s.\nThe comment given was:\n%s.\n%s%s.", what.c_str(), (deletion ? "deleted" : "banished"), formatDateEx(ban.added, "%d %b %Y").c_str(), admin.c_str(), getReason(ban.reason).c_str(), getAction(ban.action, false).c_str(), ban.comment.c_str(), end.c_str(), (deletion ? "." : formatDateEx(ban.expires).c_str()));

	player->sendFYIBox(buffer);
	return true;
}

bool TalkAction::diagnostics(
	Creature* creature,
	const std::string&,
	const std::string&
) {
