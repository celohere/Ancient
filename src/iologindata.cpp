...
bool IOLoginData::loadPlayer(Player* player, const std::string& name, bool preLoad) {
	...
	uint64_t currExpCount = Player::getExpForLevel(player->level), nextExpCount = Player::getExpForLevel(player->level + 1), experience = (uint64_t)result->getDataLong("experience");
	...
	if (rankId > 0) {
		...
		if ((result = db->storeQuery(query.str()))) {
			...
			result->free();
				...
				query.str("");
				query << "SELECT `id`, `guild_id`, `enemy_id` FROM `guild_wars` WHERE (`guild_id` = " << player->guildId << " OR `enemy_id` = " << player->guildId << ") AND `status` IN (1, 4)";
				if ((result = db->storeQuery(query.str()))) {
					War_t war;
					do {
	...
	if ((result = db->storeQuery(query.str()))) {
		...
		do {
			...
			uint64_t nextSkillCount = player->vocation->getReqSkillTries(skillId, skillLevel + 1), skillCount = result->getDataLong("count");
...
bool IOLoginData::saveItems(const Player* player, const ItemBlockList& itemList, DBInsert& query_insert) {
	...
	for (ItemBlockList::const_iterator it = itemList.begin(); it != itemList.end(); ++it, ++runningId) {
		...
		sprintf(buffer, "%d, %d, %d, %d, %d, %s", player->getGUID(), it->first, runningId, item->getID(), (int32_t)item->getSubType(), db->escapeBlob(attributes, attributesSize).c_str());
		if (!query_insert.addRow(buffer)) {
			return false;
		}
	...
	while (stackList.size()) {
		...
		for (uint32_t i = 0; i < container->size(); ++i, ++runningId) {
			...
			sprintf(buffer, "%d, %d, %d, %d, %d, %s", player->getGUID(), stack.second, runningId, item->getID(), (int32_t)item->getSubType(), db->escapeBlob(attributes, attributesSize).c_str());
			if (!query_insert.addRow(buffer)) {
				return false;
			}
		}
...
bool IOLoginData::playerDeath(Player* _player, const DeathList& dl) {
	...
	query << "INSERT INTO `player_deaths` (`player_id`, `date`, `level`) VALUES (" << _player->getGUID() << ", " << time(NULL) << ", " << _player->getLevel() << ")";
	if (!db->query(query.str())) {
		return false;
	}
	...
	for (DeathList::const_iterator it = dl.begin(); i < size && it != dl.end(); ++it, ++i) {
		...
		if (it->isCreatureKill()) {
			...
			if (player) {
				...
				query.str("");
				query << "INSERT INTO `player_killers` (`kill_id`, `player_id`) VALUES ("<< killId << ", " << player->getGUID() << ")";
				if (!db->query(query.str())) {
					return false;
				}
			} else {
		...
		if (!name.empty()) {
			query.str("");
			query << "INSERT INTO `environment_killers` (`kill_id`, `name`) VALUES (" << killId << ", " << db->escapeString(name) << ")";
			if (!db->query(query.str())) {
				return false;
			}
		}
...
bool IOLoginData::changeName(uint32_t guid, std::string newName, std::string oldName) {
	...
	query << "INSERT INTO `player_namelocks` (`player_id`, `name`, `new_name`, `date`) VALUES ("<< guid << ", " << db->escapeString(oldName) << ", " << db->escapeString(newName) << ", " << time(NULL) << ")";
	if (!db->query(query.str())) {
		return false;
	}
