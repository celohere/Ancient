...
*/
	...
	class Door : public Item {
		public:
			Door(uint16_t type):
				Item(type),	house(NULL), accessList(NULL) {}
			virtual ~Door();
	...
	class House {
		public:
			...
			void setEntry(const Position& pos) {
				entry = pos;
			}
			...
			void setLastWarning(time_t _lastWarning) {
				lastWarning = _lastWarning;
			}
			time_t getLastWarning() const {
				return lastWarning;
			}

			void setRentWarnings(uint32_t warnings) {
				rentWarnings = warnings;
			}
			uint32_t getRentWarnings() const {
				return rentWarnings;
			}
			...
			void setPendingTransfer(bool transfer) {
			...
			void setGuild(bool _guild) {
				guild = _guild;
			}
			bool isGuild() const;

			uint32_t getDoorsCount() const {
			...
			bool hasSyncFlag(syncflags_t flag) const {
				return ((syncFlags & (uint32_t)flag) == (uint32_t)flag);
			}
			void setSyncFlag(syncflags_t flag) {
				syncFlags |= (uint32_t)flag;
			}
			void resetSyncFlag(syncflags_t flag) {
				syncFlags &= ~(uint32_t)flag;
			}
			...
			HouseBedList::iterator getHouseBedsBegin() {
				return bedsList.begin();
			}
			HouseBedList::iterator getHouseBedsEnd() {
				return bedsList.end();
			}
