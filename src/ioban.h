...
*/
	...
	class IOBan {
		...
		public:
			...
			bool isAccountBanished(
				uint32_t account,
				uint32_t playerId = 0
			) const;

			bool addIpBanishment(
				uint32_t ip,
				int64_t banTime,
				uint32_t reasonId,
				std::string comment,
			...
			bool removeIpBanishment(
				uint32_t ip,
				uint32_t mask = 0xFFFFFFFF
			) const;
			bool removePlayerBanishment(
				uint32_t guid,
				PlayerBan_t type
			) const;
			bool removePlayerBanishment(
			...
			bool removeAccountBanishment(
				uint32_t account,
				uint32_t playerId = 0
			) const;
			bool removeNotations(
				uint32_t account,
				uint32_t playerId = 0
			) const;
			...
			std::vector<Ban> getList(
				Ban_t type,
				uint32_t value = 0,
				uint32_t param = 0
			);

			uint32_t getNotationsCount(
				uint32_t account,
				uint32_t playerId = 0
			) const;
			uint32_t getStatementsCount(
				uint32_t playerId,
				int16_t channelId = -1
			) const;
			uint32_t getStatementsCount(
