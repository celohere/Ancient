...
bool Monster::isOpponent(const Creature* creature) {
	return (isSummon() && master->getPlayer() && creature != master) || ((creature->getPlayer() && !creature->getPlayer()->hasFlag(PlayerFlag_IgnoredByMonsters)) || (creature->getMaster() && creature->getPlayerMaster()));
}

bool Monster::doTeleportToMaster() {
	const Position& tmp = getPosition();
	if (g_game.internalTeleport(this, g_game.getClosestFreeTile(this, master->getPosition(), true), true) != RET_NOERROR) {
		return false;
	}
...
void Monster::onCreatureLeave(Creature* creature) {
	...
	if (isSummon() && master == creature) {
		if (!g_config.getBool(ConfigManager::TELEPORT_SUMMONS) && (!master->getPlayer() || !g_config.getBool(ConfigManager::TELEPORT_PLAYER_SUMMONS))) {
	...
	switch (searchType) {
		case TARGETSEARCH_NEAREST: {
			...
			for (CreatureList::iterator it = resultList.begin(); it != resultList.end(); ++it) {
				int32_t tmp = std::max(std::abs(myPos.x - (*it)->getPosition().x), std::abs(myPos.y - (*it)->getPosition().y));
...
bool Monster::isTarget(Creature* creature) {
	return (!creature->isRemoved() && creature->isAttackable() && creature->getZone() != ZONE_PROTECTION && canSeeCreature(creature) && creature->getPosition().z == getPosition().z);
}
...
bool Monster::selectTarget(Creature* creature) {
	...
	CreatureList::iterator it = std::find(targetList.begin(), targetList.end(), creature);
	if (it == targetList.end()) { // Target not found in our target list.
		#ifdef __DEBUG__
