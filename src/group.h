...
*/
	...
	class Group {
		public:
			...
			void setAccess(uint16_t v) {
				m_access = v;
			}
			uint16_t getGhostAccess() const {
				return m_ghostAccess;
			}
			void setGhostAccess(uint16_t v) {
				m_ghostAccess = v;
			}
			uint8_t getViolationReasons() const {
				return m_violationReasons;
			}
			void setViolationReasons(uint8_t v) {
				m_violationReasons = v;
			}
			int16_t getStatementViolationFlags() const {
				return m_statementViolationFlags;
			}
			void setStatementViolationFlags(uint8_t v) {
				m_statementViolationFlags = v;
			}
			int16_t getNameViolationFlags() const {
				return m_nameViolationFlags;
			}
			void setNameViolationFlags(uint8_t v) {
				m_nameViolationFlags = v;
			}
			uint16_t getOutfit() const {
				return m_outfit;
			}
			void setOutfit(uint16_t v) {
				m_outfit = v;
			}
			...
			void setDepotLimit(uint32_t v) {
				m_depotLimit = v;
			}
			uint32_t getMaxVips(bool premium = false) const;
			void setMaxVips(uint32_t v) {
				m_maxVips = v;
			}
			...
			uint64_t getCustomFlags() const {
				return m_customFlags;
			}
			void setCustomFlags(uint64_t v) {
				m_customFlags = v;
			}

			bool hasFlag(uint64_t value) const {
				return (m_flags & ((uint64_t)1 << value));
			}
			bool hasCustomFlag(uint64_t value) const {
				return (m_customFlags & ((uint64_t)1 << value));
			}
	...
	class Groups {
		public:
			...
			GroupsMap::iterator getFirstGroup() {
				return groupsMap.begin();
			}
			GroupsMap::iterator getLastGroup() {
				return groupsMap.end();
			}
