...
bool Raids::startup() {
	...
	checkRaidsEvent = Scheduler::getInstance().addEvent(createSchedulerTask(CHECK_RAIDS_INTERVAL * 1000, boost::bind(&Raids::checkRaids, this)));
...
bool Raid::startRaid() {
	...
	nextEvent = Scheduler::getInstance().addEvent(createSchedulerTask(raidEvent->getDelay(), boost::bind(&Raid::executeRaidEvent, this, raidEvent)));
...
bool Raid::executeRaidEvent(RaidEvent* raidEvent) {
	...
	nextEvent = Scheduler::getInstance().addEvent(createSchedulerTask(std::max(RAID_MINTICKS, (int32_t)(newRaidEvent->getDelay() - raidEvent->getDelay())), boost::bind(&Raid::executeRaidEvent, this, newRaidEvent)));
	return true;
}
...
bool AnnounceEvent::configureRaidEvent(xmlNodePtr eventNode) {
	...
	if (readXMLString(eventNode, "type", strValue)) {
		...
		if (tmpStrValue == "warning") {
			...
			std::clog << "[Notice - AnnounceEvent::configureRaidEvent] Unknown type tag for announce event, using default: " << (int32_t)m_messageType << std::endl;
		}
	} else {
		std::clog << "[Notice - AnnounceEvent::configureRaidEvent] Missing type tag for announce event. Using default: " << (int32_t)m_messageType << std::endl;
	}
	return true;
}
	...
	for (MonsterSpawnList::const_iterator it = m_spawnList.begin(); it != m_spawnList.end(); it++) {
		...
		for (uint32_t i = 0; i < amount; ++i) {
			...
			for (int32_t t = 0; t < MAXIMUM_TRIES_PER_MONSTER; ++t) {
				if (!g_game.placeCreature(monster, Position(random_range(m_fromPos.x, m_toPos.x), random_range(m_fromPos.y, m_toPos.y), random_range(m_fromPos.z, m_toPos.z)), true)) {
					continue;
				}
