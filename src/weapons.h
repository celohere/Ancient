...
*/
	...
	class Weapons : public BaseEvents {
		public:
			...
			static int32_t getMaxMeleeDamage(
				int32_t attackSkill,
				int32_t attackValue
			);
			static int32_t getMaxWeaponDamage(
				int32_t level,
				int32_t attackSkill,
				int32_t attackValue,
				float attackFactor
	...
	class Weapon : public Event {
		public:
			...
			virtual int32_t playerWeaponCheck(
				Player* player,
				Creature* target
			) const;
			...
			virtual int32_t getElementDamage(
				const Player*,
				const Creature*
			) const {
				return 0;
		...
		protected:
			...
			bool executeUseWeapon(
				Player* player,
				const LuaVariant& var
			) const;

			bool internalUseWeapon(
				Player* player,
				Item* item,
				Creature* target,
			...
			virtual void onUsedWeapon(
				Player* player,
				Item* item,
				Tile* destTile
			) const;
			virtual void onUsedAmmo(
				Player* player,
				Item* item,
				Tile* destTile
			) const;
			virtual bool getSkillType(
				const Player*,
				const Item*,
				skills_t&,
				uint64_t&
