...
bool IOGuild::disbandGuild(uint32_t guildId) {
	...
	InvitedToGuildsList::iterator it;
	for (AutoList<Player>::iterator its = Player::autoList.begin(); its != Player::autoList.end(); ++its) {
		if (its->second->getGuildId() == guildId) {
			its->second->leaveGuild();
		} else {
			it = std::find(its->second->invitedToGuildsList.begin(), its->second->invitedToGuildsList.end(), guildId);
			if (it != its->second->invitedToGuildsList.end()) {
				its->second->invitedToGuildsList.erase(it);
			}
		}
	}
...
std::string IOGuild::getMotd(uint32_t guild) {
	...
	bool IOGuild::updateWar(War_t& war) {
		...
		if (war.frags[WAR_GUILD] >= war.limit || war.frags[WAR_ENEMY] >= war.limit) {
			Scheduler::getInstance().addEvent(createSchedulerTask(3000, boost::bind(&IOGuild::finishWar, this, war, true)));
			return true;
		}
	...
	void IOGuild::frag(Player* player, uint64_t deathId, const DeathList& list, bool score) {
		...
		if ((channel = g_chat.getChannel(player, CHANNEL_GUILD))) {
			...
			if (score) {
				s << " The new score is " << war.frags[war.type == WAR_GUILD] << ":" << war.frags[war.type] << " frags (limit " << war.limit << ").";
			}
		...
		if ((channel = g_chat.getChannel(list[0].getKillerCreature()->getPlayer(), CHANNEL_GUILD))) {
			...
			if (score) {
				s << " The new score is " << war.frags[war.type] << ":" << war.frags[war.type == WAR_GUILD] << " frags (limit " << war.limit << ").";
			}
			channel->talk("", SPEAK_CHANNEL_RA, s.str());
		}
