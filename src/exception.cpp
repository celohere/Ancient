...
*/
		...
		EXCEPTION_DISPOSITION __cdecl _SEHHandler(
			...
			*outdriver << "Compilation Date - " << __DATE__ << " " << __TIME__ << std::endl << std::endl;
			...
			if (GlobalMemoryStatusEx(&mstate)) {
				*outdriver << "Memory load: " << mstate.dwMemoryLoad << std::endl << "Total phys: " << mstate.ullTotalPhys/1024 << " K available phys: " << mstate.ullAvailPhys/1024 << " K" << std::endl;
			} else {
			...
			*outdriver << "Start time: " << systemtime.wDay << "-" << systemtime.wMonth << "-" << systemtime.wYear << " " << systemtime.wHour << ":" << systemtime.wMinute << ":" << systemtime.wSecond << std::endl;
			...
			*outdriver << "From: " << (uint32_t)esp << " to: " << (uint32_t)stacklimit << std::endl;
	...
	bool ExceptionHandler::LoadMap() {
			...
			FILE* input = fopen("Ancient.map", "r");
			if (!input) {
				MessageBoxA(NULL, "Failed loading symbols, Ancient.map file not found.", "Error", MB_OK | MB_ICONERROR);
				std::clog << "Failed loading symbols, Ancient.map file not found. " << std::endl;
				exit(1);
				return false;
			}
	...
	void ExceptionHandler::dumpStack() {
		...
		output << "From: " << (uint32_t)esp << " to: " << (uint32_t)stacklimit << std::endl;
