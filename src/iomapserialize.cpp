...
bool IOMapSerialize::updateAuctions() {
	...
	do {
		...
		if (!(house = Houses::getInstance()->getHouse(result->getDataInt("house_id"))) || !db->query(query.str())) {
			success = false;
			continue;
		}
...
bool IOMapSerialize::updateHouses() {
	...
	for (HouseMap::iterator it = Houses::getInstance()->getHouseBegin(); it != Houses::getInstance()->getHouseEnd(); ++it) {
		...
		query << "SELECT `price` FROM `houses` WHERE `id` = " << house->getId() << " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID) << " LIMIT 1";
		if (DBResult* result = db->storeQuery(query.str())) {
			...
			query << "`doors` = " << house->getDoorsCount() << ", `beds` = " << house->getBedsCount() << ", `tiles` = " << house->getTilesCount();
			if (house->hasSyncFlag(House::HOUSE_SYNC_GUILD)) {
				query << ", `guild` = " << house->isGuild();
			}

			query << " WHERE `id` = " << house->getId() << " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID) << db->getUpdateLimiter();
		} else {
			query.str("");
			query << "INSERT INTO `houses` (`id`, `world_id`, `owner`, `name`, `town`, `size`, `price`, `rent`, `doors`, `beds`, `tiles`, `guild`) VALUES (" << house->getId() << ", " << g_config.getNumber(ConfigManager::WORLD_ID) << ", 0, "/* we need owner for compatibility reasons (field doesn't have a default value)*/ << db->escapeString(house->getName()) << ", " << house->getTownId() << ", " << house->getSize() << ", " << house->getPrice() << ", " << house->getRent() << ", " << house->getDoorsCount() << ", " << house->getBedsCount() << ", " << house->getTilesCount() << ", " << house->isGuild() << ")";
		}
...
bool IOMapSerialize::saveHouse(Database* db, House* house) {
	...
	query.str("");
	query << "DELETE FROM `house_lists` WHERE `house_id` = " << house->getId() << " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID);
	if (!db->query(query.str())) {
		return false;
	}
	...
	if (house->getAccessList(GUEST_LIST, listText) && !listText.empty()) {
		query.str("");
		query << house->getId() << ", " << g_config.getNumber(ConfigManager::WORLD_ID) << ", " << GUEST_LIST << ", " << db->escapeString(listText);
	...
	if (house->getAccessList(SUBOWNER_LIST, listText) && !listText.empty()) {
		query.str("");
		query << house->getId() << ", " << g_config.getNumber(ConfigManager::WORLD_ID) << ", " << SUBOWNER_LIST << ", " << db->escapeString(listText);
		if (!queryInsert.addRow(query.str())) {
			return false;
		}
	}
	...
	for (HouseDoorList::iterator it = house->getHouseDoorBegin(); it != house->getHouseDoorEnd(); ++it) {
		...
		if (door->getAccessList(listText) && !listText.empty()) {
			query.str("");
			query << house->getId() << ", " << g_config.getNumber(ConfigManager::WORLD_ID) << ", " << door->getDoorId() << ", " << db->escapeString(listText);
			if (!queryInsert.addRow(query.str())) {
				return false;
			}
		}
...
bool IOMapSerialize::loadMapRelational(Map* map) {
	...
	for (HouseMap::iterator it = Houses::getInstance()->getHouseBegin(); it != Houses::getInstance()->getHouseEnd(); ++it) {
		...
		query.str("");
		query << "SELECT * FROM `tiles` WHERE `house_id` = " << house->getId() << " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID);
		if (DBResult* result = db->storeQuery(query.str())) {
			do {
				query.str("");
				query << "SELECT * FROM `tile_items` WHERE `tile_id` = " << result->getDataInt("id") << " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID) << " ORDER BY `sid` DESC";
				if (DBResult* itemsResult = db->storeQuery(query.str())) {
					if (house->hasPendingTransfer()) {
						...
						if (Tile* tile = map->getTile(pos)) {
							...
							std::clog << "[Error - IOMapSerialize::loadMapRelational] Unserialization" << " of invalid tile at position "<< pos << std::endl;
						}
					}
					itemsResult->free();
				}
			...
			for (HouseTileList::iterator it = house->getHouseTileBegin(); it != house->getHouseTileEnd(); ++it) {
				query.str("");
				query << "SELECT `id` FROM `tiles` WHERE `x` = " << (*it)->getPosition().x << " AND `y` = " << (*it)->getPosition().y << " AND `z` = " << (*it)->getPosition().z << " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID) << " LIMIT 1";
				if (DBResult* result = db->storeQuery(query.str())) {
					query.str("");
					query << "SELECT * FROM `tile_items` WHERE `tile_id` = " << result->getDataInt("id") << " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID) << " ORDER BY `sid` DESC";
					if (DBResult* itemsResult = db->storeQuery(query.str())) {
	...
	do {
		...
		while (propStream.size()) {
			...
			if (house && house->hasPendingTransfer()) {
				...
				std::clog << "[Error - IOMapSerialize::loadMapBinary] Unserialization of invalid tile" << " at position " << pos << std::endl;
				break;
			}
		}
	} while (result->next());
...
bool IOMapSerialize::saveMapBinary(Map*) {
	...
	for (HouseMap::iterator it = Houses::getInstance()->getHouseBegin(); it != Houses::getInstance()->getHouseEnd(); ++it) {
		...
		query << it->second->getId() << ", " << g_config.getNumber(ConfigManager::WORLD_ID) << ", " << db->escapeBlob(attributes, attributesSize);
...
bool IOMapSerialize::loadItems(Database*, DBResult* result, Cylinder* parent, bool depotTransfer) {
	...
	do {
		...
		if (item) {
			...
			if (!depotTransfer) {
				std::clog << "[Warning - IOMapSerialize::loadItems] NULL item at " << tile->getPosition() << " (type = " << id << ", sid = " << sid << ", pid = " << pid << ")" << std::endl;
			} else {
...
bool IOMapSerialize::saveItems(Database* db, uint32_t& tileId, uint32_t houseId, const Tile* tile) {
	...
	for (int32_t i = 0; i < thingCount; ++i) {
		...
		if (!stored) {
			...
			query << "INSERT INTO `tiles` (`id`, `world_id`, `house_id`, `x`, `y`, `z`) VALUES (" << tileId << ", " << g_config.getNumber(ConfigManager::WORLD_ID) << ", " << houseId << ", " << tilePosition.x << ", " << tilePosition.y << ", " << tilePosition.z << ")";
			if (!db->query(query.str())) {
				return false;
			}
		...
		query << tileId << ", " << g_config.getNumber(ConfigManager::WORLD_ID) << ", " << ++runningId << ", " << parentId << ", " << item->getID() << ", " << (int32_t)item->getSubType() << ", " << db->escapeBlob(attributes, attributesSize);
...
bool IOMapSerialize::saveTile(PropWriteStream& stream, const Tile* tile) {
	...
	for (; tileCount > 0; --tileCount) {
		if ((item = tile->__getThing(tileCount - 1)->getItem()) && (item->isMovable() || item->forceSerialize())) {
