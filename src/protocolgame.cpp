...
void ProtocolGame::addGameTaskInternal(
	uint32_t delay,
...
bool ProtocolGame::login(
	...
	if (!_player || name == "Account Manager" || g_config.getNumber(ConfigManager::ALLOW_CLONES) > (int32_t)players.size()) {
		...
		if (IOBan::getInstance()->getData(ban) && !player->hasFlag(PlayerFlag_CannotBeBanned)) {
			...
			sprintf(buffer, "Your character has been %s at:\n%s by: %s,\nfor the following reason:\n%s.\nThe action taken was:\n%s.\nThe comment given was:\n%s.\nYour %s%s.", (deletion ? "deleted" : "banished"), formatDateEx(ban.added, "%d %b %Y").c_str(), name_.c_str(), getReason(ban.reason).c_str(), getAction(ban.action, false).c_str(), ban.comment.c_str(), (deletion ? "character won't be undeleted" : "banishment will be lifted at:\n"), (deletion ? "" : formatDateEx(ban.expires).c_str()));
...
void ProtocolGame::disconnectClient(
	uint8_t error,
	const char* message
) {
	if (OutputMessage_ptr output = OutputMessagePool::getInstance()->getOutputMessage(this, false)) {
...
bool ProtocolGame::parseFirstPacket(NetworkMessage& msg) {
	...
	uint32_t key[4] = {
		msg.get<uint32_t>(), msg.get<uint32_t>(), msg.get<uint32_t>(), msg.get<uint32_t>()
	};
	...
	if (IOBan::getInstance()->getData(ban) && !IOLoginData::getInstance()->hasFlag(id, PlayerFlag_CannotBeBanned)) {
		...
		char buffer[500 + ban.comment.length()];
		sprintf(buffer, "Your account has been %s at:\n%s by: %s,\nfor the following reason:\n%s.\nThe action taken was:\n%s.\nThe comment given was:\n%s.\nYour %s%s.", (deletion ? "deleted" : "banished"), formatDateEx(ban.added, "%d %b %Y").c_str(), name_.c_str(), getReason(ban.reason).c_str(), getAction(ban.action, false).c_str(), ban.comment.c_str(), (deletion ? "account won't be undeleted" : "banishment will be lifted at:\n"), (deletion ? "" : formatDateEx(ban.expires).c_str()));
...
void ProtocolGame::parsePacket(NetworkMessage &msg) {
	...
	if (player->isAccountManager()) {
		switch (recvbyte) {
			...
			default: sendCancelWalk(); break;
		}
	} else {
		switch (recvbyte) {
			case 0x14: parseLogout(msg); break;
			case 0x1E: parseReceivePing(msg); break; // keep alive / ping response

			case 0x64: parseAutoWalk(msg); break; // move with steps
			case 0x65: // move north
			case 0x66: // move east
			case 0x67: // move south
			case 0x68: parseMove(msg, (Direction)(recvbyte - 0x65)); break; // move west
			case 0x69: addGameTask(&Game::playerStopAutoWalk, player->getID()); break; // stop-autowalk
			...
			case 0x72: parseTurn(msg, (Direction)(recvbyte - 0x6F)); break; // turn west
			...
			case 0x9A: parseOpenPriv(msg); break;
			...
			case 0xC9: parseUpdateTile(msg); break; // client request to resend the tile
			case 0xCA: parseUpdateContainer(msg); break; // client request to resend the container (happens when you store more than container maxsize)
			...
			case 0xD3: {
				if ((!player->hasCustomFlag(PlayerCustomFlag_GamemasterPrivileges) || !g_config.getBool(ConfigManager::DISABLE_OUTFITS_PRIVILEGED)) && (g_config.getBool(ConfigManager::ALLOW_CHANGECOLORS) || g_config.getBool(ConfigManager::ALLOW_CHANGEOUTFIT))) {
					parseSetOutfit(msg);
				}
				break;
			}
			...
			default: {
				if (g_config.getBool(ConfigManager::BAN_UNKNOWN_BYTES)) {
					...
					if (IOBan::getInstance()->addAccountBanishment(tmp.number, banTime, 13, action, "Sending unknown packets to the server.", 0, player->getGUID())) {
				...
				std::stringstream ss;
				ss << "0x" << std::hex << (int16_t)recvbyte << std::dec;
				Logger::getInstance()->eFile(getFilePath(FILE_TYPE_LOG, "bots/" + player->getName() + ".log").c_str(), "[" + formatDate() + "] Received byte " + ss.str(), false);
				break;
			}
		}
	}
...
bool ProtocolGame::canSee(
	...
	return ((x >= myPos.x - 8 + offsetz) && (x <= myPos.x + 9 + offsetz) && (y >= myPos.y - 6 + offsetz) && (y <= myPos.y + 7 + offsetz));
}
...
void ProtocolGame::parseDebugAssert(NetworkMessage& msg) {
	...
	std::stringstream s;
	s << "----- " << formatDate() << " - " << player->getName() << " (" << convertIPAddress(getIP()) << ") -----" << std::endl << msg.getString() << std::endl << msg.getString() << std::endl << msg.getString() << std::endl << msg.getString() << std::endl << std::endl;
...
void ProtocolGame::sendCreatureOutfit(
...
void ProtocolGame::sendCreatureSquare(
	const Creature* creature,
	uint8_t color
) {
	if (!canSee(creature)) {
...
void ProtocolGame::sendAddMarker(
	const Position& pos,
	MapMarks_t markType,
...
void ProtocolGame::sendTextMessage(
...
void ProtocolGame::sendContainer(
	uint32_t cid,
...
void ProtocolGame::sendTradeItemRequest(
	const Player* player,
	const Item* item,
	bool ack
) {
...
void ProtocolGame::sendCreatureTurn(
	const Creature* creature,
	int16_t stackpos
) {
...
void ProtocolGame::sendDistanceShoot(
	const Position& from,
	const Position& to,
	uint8_t type
) {
...
void ProtocolGame::sendMagicEffect(
	const Position& pos,
	uint8_t type
) {
...
void ProtocolGame::sendAnimatedText(
	const Position& pos,
	uint8_t color,
	std::string text
) {
...
// tile
void ProtocolGame::sendAddTileItem(
	const Tile*,
	const Position& pos,
	uint32_t stackpos,
	const Item* item
...
void ProtocolGame::sendUpdateTileItem(
	const Tile*,
	const Position& pos,
	uint32_t stackpos,
	const Item* item
...
void ProtocolGame::sendRemoveTileItem(
	const Tile*,
	const Position& pos,
	uint32_t stackpos
) {
...
void ProtocolGame::sendUpdateTile(
	const Tile* tile,
	const Position& pos
) {
	if (!canSee(pos)) {
...
void ProtocolGame::sendRemoveCreature(
	const Creature*,
	const Position& pos,
	uint32_t stackpos
) {
...
// inventory
void ProtocolGame::sendAddInventoryItem(
	slots_t slot,
	const Item* item
) {
	NetworkMessage_ptr msg = getOutputBuffer();
...
void ProtocolGame::sendUpdateInventoryItem(
	slots_t slot,
	const Item* item
) {
	NetworkMessage_ptr msg = getOutputBuffer();
...
// containers
void ProtocolGame::sendAddContainerItem(
	uint8_t cid,
	const Item* item
) {
	NetworkMessage_ptr msg = getOutputBuffer();
...
void ProtocolGame::sendUpdateContainerItem(
	uint8_t cid,
	uint8_t slot,
	const Item* item
) {
...
void ProtocolGame::sendRemoveContainerItem(
	uint8_t cid,
	uint8_t slot
) {
	NetworkMessage_ptr msg = getOutputBuffer();
...
void ProtocolGame::sendTextWindow(
...
void ProtocolGame::sendHouseWindow(
	uint32_t windowTextId,
	House*,
	uint32_t,
	const std::string& text
...
void ProtocolGame::AddMapDescription(
	NetworkMessage_ptr msg,
	const Position& pos
) {
	msg->put<char>(0x64);
...
void ProtocolGame::AddTextMessage(
	NetworkMessage_ptr msg,
	MessageClasses mclass,
	const std::string& message
) {
...
void ProtocolGame::AddAnimatedText(
	NetworkMessage_ptr msg,
	const Position& pos,
	uint8_t color,
	const std::string& text
...
void ProtocolGame::AddDistanceShoot(
	NetworkMessage_ptr msg,
	const Position& from,
	const Position& to,
	uint8_t type
...
void ProtocolGame::AddCreatureSpeak(
	NetworkMessage_ptr msg,
	const Creature* creature,
	SpeakClasses type,
	std::string text,
...
void ProtocolGame::AddCreatureHealth(
	NetworkMessage_ptr msg,
	const Creature* creature
) {
	msg->put<char>(0x8C);
...
void ProtocolGame::AddCreatureOutfit(
	NetworkMessage_ptr msg,
	const Creature* creature,
	const Outfit_t& outfit,
	bool outfitWindow
) {
	if (outfitWindow || !creature->getPlayer() || (!creature->isInvisible() && (!creature->isGhost() || !g_config.getBool(ConfigManager::GHOST_INVISIBLE_EFFECT)))) {
...
void ProtocolGame::AddWorldLight(
	NetworkMessage_ptr msg,
...
void ProtocolGame::AddCreatureLight(
	NetworkMessage_ptr msg,
	const Creature* creature
) {
	msg->put<char>(0x8D);
...
void ProtocolGame::AddTileCreature(
	NetworkMessage_ptr msg,
	const Position& pos,
	uint32_t stackpos,
	const Creature* creature
...
void ProtocolGame::UpdateTileItem(
	NetworkMessage_ptr msg,
	const Position& pos,
	uint32_t stackpos,
	const Item* item
...
void ProtocolGame::RemoveTileItem(
	NetworkMessage_ptr msg,
	const Position& pos,
	uint32_t stackpos
) {
...
// inventory
void ProtocolGame::AddInventoryItem(
	NetworkMessage_ptr msg,
	slots_t slot,
	const Item* item
) {
...
void ProtocolGame::RemoveInventoryItem(
	NetworkMessage_ptr msg,
	slots_t slot
) {
	msg->put<char>(0x79);
...
void ProtocolGame::UpdateInventoryItem(
	NetworkMessage_ptr msg,
	slots_t slot,
	const Item* item
) {
...
// containers
void ProtocolGame::AddContainerItem(
	NetworkMessage_ptr msg,
	uint8_t cid,
	const Item* item
) {
...
void ProtocolGame::UpdateContainerItem(
	NetworkMessage_ptr msg,
	uint8_t cid,
	uint8_t slot,
	const Item* item
...
void ProtocolGame::RemoveContainerItem(
	NetworkMessage_ptr msg,
	uint8_t cid,
	uint8_t slot
) {
