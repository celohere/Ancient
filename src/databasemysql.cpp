...
DatabaseMySQL::DatabaseMySQL():
	m_timeoutTask(0) {
		...
		if (timeout) {
			m_timeoutTask = Scheduler::getInstance().addEvent(createSchedulerTask(timeout, boost::bind(&DatabaseMySQL::keepAlive, this)));
		}
		...
		if (result->getDataLong("Value") < 16776192) {
			std::clog << std::endl << "> WARNING: max_allowed_packet might be set too low for binary map storage." << std::endl << "Use the following query to raise max_allow_packet: SET GLOBAL max_allowed_packet = 16776192;" << std::endl;
		}
		result->free();
	}
...
void DatabaseMySQL::keepAlive() {
	...
	Scheduler::getInstance().addEvent(createSchedulerTask(timeout, boost::bind(&DatabaseMySQL::keepAlive, this)));
}
