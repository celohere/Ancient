					...
					while (subNode) {
						if (!xmlStrcmp(subNode->name, (const xmlChar*)"action")) {
							...
							if (subResponseList.empty()) {
								...
								subResponseList.insert(subResponseList.end(), nodeResponseList.begin(), nodeResponseList.end());
							}
						}
					...
					if (hasListKeyword && !prop.itemList.empty()) {
						for (std::list<ListItem>::iterator it = prop.itemList.begin(); it != prop.itemList.end(); ++it) {
							...
							for (std::list<std::string>::iterator iit = listItemProp.inputList.begin();
								...
								listItemProp.actionList.push_front(action);
								ResponseList list;
								for (ResponseList::iterator rit = subResponseList.begin(); rit != subResponseList.end(); ++rit) {
...
void Npc::onCreatureSay(
	...
	if (const Player* player = creature->getPlayer()) {
		...
		if (type == SPEAK_SAY || type == SPEAK_PRIVATE_PN) {
			...
			if (canSee(myPos) && (destPos.x >= myPos.x - talkRadius) && (destPos.x <= myPos.x + talkRadius) && (destPos.y >= myPos.y - talkRadius) && (destPos.y <= myPos.y + talkRadius)) {
				if (NpcState* npcState = getState(player)) {
				...
				case ACTION_SCRIPT: {
					...
					if (interface.reserveEnv()) {
						...
						for (std::list<ListItem>::const_iterator iit = response->prop.itemList.begin(); iit != response->prop.itemList.end(); ++iit) {
							scriptstream << "{id = " << iit->itemId << ", subType = " << iit->subType << ", buy = " << iit->buyPrice << ", sell = " << iit->sellPrice<< ", name = '" << iit->name << "'}";
...
const NpcResponse* Npc::getResponse(
	...
	for (ResponseList::const_iterator it = list.begin(); it != list.end(); ++it) {
		...
		if ((*it)->getInteractType() == INTERACT_EVENT) {
			...
			int32_t matchAllCount = 0, totalKeywordCount = 0, matchWordCount = getMatchCount(*it, wordList, exactMatch, matchAllCount, totalKeywordCount);
			if (matchWordCount> 0) {
