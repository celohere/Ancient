...
void PrivateChatChannel::invitePlayer(
	...
	std::stringstream ss;
	ss << player->getName() << " invites you to " << (player->getSex(false) ? "his" : "her") << " private chat channel.";
	invitePlayer->sendTextMessage(MSG_INFO_DESCR, ss.str().c_str());
...
void PrivateChatChannel::excludePlayer(
	...
	std::string s = excludePlayer->getName();
	s += " has been excluded.";
	player->sendTextMessage(MSG_INFO_DESCR, s.c_str());
...
ChatChannel* Chat::getChannel(
	...
	if (nit != m_normalChannels.end()) {
		...
		if (!tmpChannel || !tmpChannel->hasFlag(CHANNELFLAG_ENABLED) || player->getAccess() < tmpChannel->getAccess() || (!player->hasCustomFlag(PlayerCustomFlag_GamemasterPrivileges) && !tmpChannel->checkVocation(player->getVocationId()))) {
			#ifdef __DEBUG_CHAT__
