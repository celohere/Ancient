...
bool argumentsHandler(StringVec args) {
	...
	for (StringVec::iterator it = args.begin(); it != args.end(); ++it) {
		...
		if ((*it) == "--version" || (*it) == "-V") {
			...
			"A server developed by contributors of The Forgotten Server.\n"
			"Visit the repository for updates, support and resources: http://github.com/ramon-bernardo/Ancient.\n";
			return false;
		}
	...
	void signalHandler(int32_t sig) {
		switch (sig) {
			case SIGHUP: {
				Dispatcher::getInstance().addTask(createTask(boost::bind(&Game::saveGameState, &g_game, false)));
				break;
			}
			...
			case SIGCONT: {
				Dispatcher::getInstance().addTask(createTask(boost::bind(&Game::reloadInfo, &g_game, RELOAD_ALL, 0)));
				break;
			}
			...
			case SIGTERM: {
				Dispatcher::getInstance().addTask(createTask(boost::bind(&Game::shutdown, &g_game)));
				break;
			}
...
void otserv(StringVec, ServiceManager* services) {
	...
	g_game.setGameState(GAMESTATE_STARTUP);
		...
		if (!getuid() || !geteuid()) {
			std::clog << "> WARNING: " << SOFTWARE_NAME << " has been executed as super user! It is " << "recommended to run as a normal user." << std::endl << "Continue? (y/N)" << std::endl;
			char buffer = getch();
	...
	std::clog << SOFTWARE_NAME << ", version " << SOFTWARE_VERSION << " (" << SOFTWARE_CODENAME << ")" << std::endl
		...
		<< "A server developed by contributors of The Forgotten Server." << std::endl
		<< "Visit the repository for updates, support and resources: http://github.com/ramon-bernardo/Ancient." << std::endl << std::endl;
	std::stringstream ss;
	...
	if (encryptionType == "md5") {
		...
		std::clog << "> Using plaintext encryption" << std::endl << std::endl << "> WARNING: This method is completely unsafe!" << std::endl << "> Please set encryptionType = \"sha1\" (or any other available method) in config.lua" << std::endl;
		...
		if (!xmlStrcmp(root->name, (const xmlChar*)"versions")) {
			...
			if (!xmlStrcmp(p->name, (const xmlChar*)"entry")) {
				...
				if (tmp) {
					...
					std::clog << std::endl << "> Current version information - version: " << SOFTWARE_VERSION << ", patch: " << VERSION_PATCH << ", build: " << VERSION_BUILD << ", timestamp: " << VERSION_TIMESTAMP << "." << std::endl << "> Latest version information - version: " << version << ", patch: " << patch << ", build: " << build << ", timestamp: " << timestamp << "." << std::endl;

					if (g_config.getBool(ConfigManager::CONFIRM_OUTDATED_VERSION) && asLowerCaseString(version).find("_svn") == std::string::npos) {
		...
		if (!gethostname(hostName, 128)) {
			if (hostent* host = gethostbyname(hostName)) {
				...
				for (uint8_t** addr = (uint8_t**)host->h_addr_list; addr[0] != NULL; addr++) {
					...
					s << (int32_t)(addr[0][0]) << "." << (int32_t)(addr[0][1]) << "." << (int32_t)(addr[0][2]) << "." << (int32_t)(addr[0][3]) << "\t";
				}
