...
bool Monsters::loadFromXml(bool reloading) {
	loaded = false;
...
bool Monsters::deserializeSpell(
	...
	if (isScripted) {
		...
		if (tmpName == "melee" || tmpName == "distance") {
			...
			if (ConditionSpeed* condition = dynamic_cast<ConditionSpeed*>(Condition::createCondition(CONDITIONID_COMBAT, conditionType, duration))) {
		...
		} else if (tmpName == "firecondition" || tmpName == "energycondition" || tmpName == "drowncondition" || tmpName == "poisoncondition" || tmpName == "earthcondition" || tmpName == "freezecondition" || tmpName == "cursecondition" || tmpName == "dazzlecondition") {
			ConditionType_t conditionType = CONDITION_NONE;
			...
			while (tmpNode) {
				if (!xmlStrcmp(tmpNode->name, (const xmlChar*)"immunity")) {
					if (readXMLString(tmpNode, "name", strValue)) {
						...
						mType->damageImmunities |= COMBAT_FIREDAMAGE;
						mType->conditionImmunities |= CONDITION_FIRE;
					} else if ((readXMLString(tmpNode, "poison", strValue) || readXMLString(tmpNode, "earth", strValue)) && booleanString(strValue)) {
						mType->damageImmunities |= COMBAT_EARTHDAMAGE;
						mType->conditionImmunities |= CONDITION_POISON;
