...
bool Map::placeCreature(const Position& centerPos, Creature* creature, bool extendedPos, bool forced) {
...
void Map::getSpectators(SpectatorVec& list, const Position& centerPos, bool checkforduplicate, bool multifloor, int32_t minRangeX, int32_t maxRangeX, int32_t minRangeY, int32_t maxRangeY) {
	...
	if (!foundCache) {
		...
		getSpectatorsInternal(list, centerPos, true, minRangeX,	maxRangeX, minRangeY, maxRangeY, minRangeZ, maxRangeZ);
...
const SpectatorVec& Map::getSpectators(const Position& centerPos) {
	...
	int32_t minRangeX = -maxViewportX, maxRangeX = maxViewportX, minRangeY = -maxViewportY, maxRangeY = maxViewportY, minRangeZ, maxRangeZ;
	if (centerPos.z > 7) {
		// underground, 8->15
		minRangeZ = std::max(centerPos.z - 2, 0);
		maxRangeZ = std::min(centerPos.z + 2, MAP_MAX_LAYERS - 1);
...
bool Map::canThrowObjectTo(const Position& fromPos, const Position& toPos, bool checkLineOfSight, int32_t rangex, int32_t rangey) {
	// z checks
	// underground 8->15
	// ground level and above 7->0
	if ((fromPos.z >= 8 && toPos.z < 8) || (toPos.z >= 8 &&	fromPos.z < 8) || fromPos.z - fromPos.z > 2) {
		return false;
	}

	int32_t deltax = std::abs(fromPos.x - toPos.x), deltay = std::abs(fromPos.y - toPos.y), deltaz = std::abs(fromPos.z - toPos.z);
...
bool Map::checkSightLine(const Position& fromPos, const Position& toPos) const {
	...
	int32_t x, y, z, dx = std::abs(start.x - end.x), dy = std::abs(start.y - end.y), dz = std::abs(start.z - end.z), sx, sy, sz, ey, ez, max = dx, dir = 0;
	if (dy > max) {
		max = dy;
		dir = 1;
	}
...
bool Map::getPathTo(const Creature* creature, const Position& destPos, std::list<Direction>& listDir, int32_t maxSearchDist) {
	...
	while (maxSearchDist != -1 || nodes.countClosedNodes() < 100) {
		...
		for (uint8_t i = 0; i < 8; ++i) {
			...
			if (!outOfRange && (tile = canWalkTo(creature, pos))) {
				...
				int32_t cost = nodes.getMapWalkCost(creature, n, tile, pos), extraCost = nodes.getTileWalkCost(creature, tile), newg = n->g + cost + extraCost;
	...
	while (fpp.maxSearchDist != -1 || nodes.countClosedNodes() < 100) {
		...
		for (int32_t i = 0; i < dirCount; ++i) {
			...
			if (inRange && (tile = canWalkTo(creature, pos))) {
				// The cost (g) for this neighbour
				int32_t cost = nodes.getMapWalkCost(creature, n, tile, pos), extraCost = nodes.getTileWalkCost(creature, tile), newf = n->f + cost + extraCost;
...
int32_t AStarNodes::getMapWalkCost(const Creature*, AStarNode* node, const Tile*, const Position& neighbourPos) {
...
int32_t AStarNodes::getEstimatedDistance(uint16_t x, uint16_t y, uint16_t xGoal, uint16_t yGoal) {
	...
	return (MAP_DIAGONALWALKCOST * diagonal) + (MAP_NORMALWALKCOST * ((std::abs(x - xGoal) + std::abs(y - yGoal)) - (2 * diagonal)));
}
