...
*/
	...
	class ItemType {
		...
		public:
			...
			bool hasSubType() const {
				return (isFluidContainer() || isSplash() || stackable || charges);
			}

			bool stopTime, showCount, stackable, showDuration, showCharges, showAttributes, dualWield, allowDistRead, canReadText, canWriteText, forceSerialize, isVertical, isHorizontal, isHangable, usable, movable, pickupable, rotable, replacable, lookThrough, walkStack, hasHeight, blockSolid, blockPickupable, blockProjectile, blockPathFind, allowPickupable, alwaysOnTop, floorChange[CHANGE_LAST];
			...
			uint16_t transformUseTo[2], transformToFree, transformEquipTo, transformDeEquipTo, id, clientId, maxItems, slotPosition, wieldPosition, speed, maxTextLength, writeOnceItemId;
			...
			uint32_t shootRange, charges, decayTime, attackSpeed, wieldInfo, minReqLevel, minReqMagicLevel, worth, levelDoor, date;
	...
	class Items {
		public:
			Items():
				m_randomizationChance(RANDOMIZATION), items(ITEMS) {}
			virtual ~Items() {
				clear();
			}
			...
			uint8_t getRandomizationChance() const {
				return m_randomizationChance;
			}
			const RandomizationBlock getRandomization(int16_t id) {
				return randomizationMap[id];
			}

			uint32_t size() {
				return items.size();
			}
			const IntegerMap getMoneyMap() const {
				return moneyMap;
			}
