...
std::string transformToVAHash(
	...
	CryptoPP::HMAC<CryptoPP::SHA512>((const byte*)key.c_str(), key.length()).CalculateDigest(digest, (const byte*)sha256.c_str(), CryptoPP::SHA256::DIGESTSIZE);
...
std::string getFilePath(
	...
	switch (type) {
		...
		case FILE_TYPE_CONFIG: {
				...
				if (fileExists("~/.ancient/" + name)) {
					path = "~/.ancient/" + name;
				}
			#else
				#if defined(__FILESYSTEM_HIERARCHY_STANDARD__)
					path = "/etc/ancient/" + name;
				#else
					path = name;
				#endif
			#endif
