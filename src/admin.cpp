...
*/
			...
			case AP_MSG_COMMAND: {
				...
				switch (command) {
					...
					case CMD_SHALLOW_SAVE_SERVER: {
						...
						Dispatcher::getInstance().addTask(createTask(boost::bind(&Game::saveGameState, &g_game, (command == CMD_SHALLOW_SAVE_SERVER))));
					...
					case CMD_KICK: { // why do we run these below on dispatcher thread anyway?
	...
	Admin::Admin():
		m_currrentConnections(0), m_encrypted(false), m_key_RSA1024XTEA(NULL) {
