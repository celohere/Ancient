...
bool Creature::canSee(
	...
	return ((uint32_t)pos.x >= myPos.x - viewRangeX + offsetz) &&
			((uint32_t)pos.x <= myPos.x + viewRangeX + offsetz) &&
			((uint32_t)pos.y >= myPos.y - viewRangeY + offsetz) &&
			((uint32_t)pos.y <= myPos.y + viewRangeY + offsetz);
}
...
void Creature::addEventWalk(bool firstStep) {
	...
	eventWalk = Scheduler::getInstance().addEvent(createSchedulerTask(std::max((int64_t)SCHEDULER_MINTICKS, ticks), boost::bind(&Game::checkCreatureWalk, &g_game, id)));
}
...
void Creature::onRemoveTileItem(
	const Tile* tile,
	const Position& pos,
...
void Creature::internalCreatureDisappear(
...
DeathList Creature::getKillers() {
	...
	for (it = damageMap.begin(); it != damageMap.end(); ++it) {
		...
		for (DeathList::iterator fit = list.begin(); fit != list.end(); ++fit) {
			...
			if (!(mdc->getName() == tmp->getName() && mdc->getMaster() == tmp->getMaster()) && (!mdc->getMaster() || (mdc->getMaster() != tmp && mdc->getMaster() != tmp->getMaster())) && (mdc->getSummonCount() <= 0 || tmp->getMaster() != mdc)) {
				continue;
			}
...
void Creature::getPathSearchParams(
	const Creature*,
	FindPathParams& fpp
) const {
...
bool Creature::setFollowCreature(
	Creature* creature,
	bool
) {
	if (creature) {
...
void Creature::addDamagePoints(
	Creature* attacker,
	int32_t damagePoints
) {
...
void Creature::onTickCondition(
	ConditionType_t type,
	int32_t, bool& _remove
) {
...
void Creature::onCombatRemoveCondition(
	const Creature*,
	Condition* condition
) {
...
void Creature::onAttackedCreatureDrainHealth(
	Creature* target,
	int32_t points
) {
...
void Creature::onAttackedCreatureDrainMana(
	Creature* target,
	int32_t points
) {
	onAttackedCreatureDrain(target, points);
...
void Creature::onAttackedCreatureDrain(
	Creature* target,
	int32_t points
) {
...
void Creature::onTargetCreatureGainHealth(
	Creature* target,
	int32_t points
) {
...
bool Creature::onKilledCreature(
	Creature* target,
	DeathEntry& entry
) {
	bool ret = true;
...
void Creature::onGainExperience(
	double& gainExp,
	bool fromMonster,
	bool multiplied
) {
...
void Creature::onGainSharedExperience(
	double& gainExp,
	bool fromMonster,
	bool multiplied
) {
...
void Creature::removeConditions(
...
bool FrozenPathingConditionCall::isInRange(
