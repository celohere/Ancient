...
ReturnValue Actions::canUse(
	...
	} else if ((action = getAction(item, ACTION_ACTIONID))) {
		return action->canExecuteAction(player, pos);
	} else if ((action = getAction(item, ACTION_ITEMID))) {
		return action->canExecuteAction(player, pos);
	} else if ((action = getAction(item, ACTION_RUNEID))) {
		return action->canExecuteAction(player, pos);
...
ReturnValue Actions::canUseFar(
	...
	if (creaturePos.z > toPos.z) {
		...
		return RET_TOOFARAWAY;
	} else if (checkLineOfSight && !g_game.canThrowObjectTo(creaturePos, toPos)) {
...
ReturnValue Actions::internalUseItem(
	...
	if (Container* container = item->getContainer()) {
		if (container->getCorpseOwner() && !player->canOpenCorpse(container->getCorpseOwner()) && g_config.getBool(ConfigManager::CHECK_CORPSE_OWNER)) {
...
bool Action::executeUse(
	...
	if (m_interface->reserveEnv()) {
		...
		if (m_scripted == EVENT_SCRIPT_BUFFER) {
			...
			env->streamPosition(ss, "fromPosition", fromPos, fromPos.stackpos);

			Thing* thing = g_game.internalGetThing(player, toPos, toPos.stackpos);
			if (thing && (thing != item || !extendedUse)) {
				env->streamThing(ss, "itemEx", thing, env->addThing(thing));
				env->streamPosition(ss, "toPosition", toPos, toPos.stackpos);
			} else {
				env->streamThing(ss, "itemEx", NULL, 0);
				env->streamPosition(ss, "toPosition", PositionEx());
			}
			...
			return result;
				...
				std::stringstream ss;
				ss << player->getName() << " - " << item->getID() << " " << fromPos << "|" << toPos;
				env->setEvent(ss.str());
			#endif
