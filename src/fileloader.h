...
*/
	...
	class PropStream {
		public:
			...
			inline bool getByte(uint8_t& ret) {
				return getType(ret);
			}
			inline bool getShort(uint16_t& ret) {
				return getType(ret);
			}
			inline bool getTime(time_t& ret) {
				return getType(ret);
			}
			inline bool getLong(uint32_t& ret) {
				return getType(ret);
			}
	...
	class PropWriteStream {
		public:
			...
			inline void addByte(uint8_t ret) {
				addType(ret);
			}
			inline void addShort(uint16_t ret) {
				addType(ret);
			}
			inline void addTime(time_t ret) {
				addType(ret);
			}
			inline void addLong(uint32_t ret) {
				addType(ret);
			}
