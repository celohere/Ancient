	...
	class Item : virtual public Thing, public ItemAttributes {
		public:
			...
			std::string getWeightDescription() const {
				return getWeightDescription(getWeight(), items[id].stackable && items[id].showCount, count);
			}
			...
			void setDate(time_t date) {
				setAttribute("date", (int32_t)date);
			}
			void resetDate() {
				eraseAttribute("date");
			}
			...
			int32_t getMaxWriteLength() const {
				return items[id].maxTextLength;
			}
			int32_t getWorth() const {
				return getItemCount() * items[id].worth;
			}
			...
			bool forceSerialize() const {
				return items[id].forceSerialize || canWriteText() || isContainer() || isBed() || isDoor();
			}
			...
			virtual bool isBlocking(const Creature*) const {
			...
			bool isTeleport() const {
				return items[id].isTeleport();
			}
			bool isKey() const {
				return items[id].isKey();
			}
			bool isDepot() const {
				return items[id].isDepot();
			}
			bool isMailbox() const {
				return items[id].isMailbox();
			}
			bool isTrashHolder() const {
				return items[id].isTrashHolder();
			}
			bool isBed() const {
