...
*/
	...
	class Mailbox : public Item, public Cylinder {
		public:
			...
			virtual void postAddNotification(Creature* actor, Thing* thing, const Cylinder* oldParent, int32_t index, cylinderlink_t = LINK_OWNER) {
				if (getParent()) {
					getParent()->postAddNotification(actor, thing, oldParent, index, LINK_PARENT);
				}
			}
			virtual void postRemoveNotification(Creature* actor, Thing* thing, const Cylinder* newParent, int32_t index, bool isCompleteRemoval, cylinderlink_t = LINK_OWNER) {
				if (getParent()) {
